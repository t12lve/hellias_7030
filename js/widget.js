(function (global) {
      ...
      '<div class="widget-shell size-M" id="widget-shell">' +
        ...
        '<div class="widget-content">' +
          '<div class="widget-header">' +
            '<span class="widget-label">70/30</span>' +
            '<span class="widget-counter"><span class="current">0</span><span class="sep">/</span><span class="goal">300</span></span>' +
          '</div>' +
  ...
  Widget.prototype.applyConfig = function (cfg) {
    ...
    this.label.textContent = cfg.label || '70/30';
    this.goalEl.textContent = String(cfg.goal || 300);
  ...
  Widget.prototype.render = function (animate) {
    ...
    var goal = Number(cfg.goal) || 300;
    var pct = goal > 0 ? Math.min(100, (points / goal) * 100) : 0;
    var streak = Store.computeStreak(cfg, points);

    if (animate && this.prevPoints !== null) {
    ...
    if (this.prevStreak !== null && streak > this.prevStreak) {
      this.anims.playStreakGain();
    }

    this.fill.style.width = pct + '%';
    this.anims.resetMilestone(pct);
  ...
  Widget.prototype.runDemoTick = function () {
    var goal = Number(this.cfg.goal) || 300;
